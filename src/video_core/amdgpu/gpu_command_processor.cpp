...
#define MAX_NAMES 56
static_assert(GpuCommandProcessor::NumComputeRings <= MAX_NAMES);
...
std::array<u8, 48_KB> GpuCommandProcessor::ConstantEngine::constants_heap;
...
GpuCommandProcessor::GpuCommandProcessor() {
    ...
    process_thread = std::jthread{std::bind_front(&GpuCommandProcessor::Process, this)};
}
...
void GpuCommandProcessor::Enqueue(Common::UniqueFunction<void> command, bool wait) {
    ...
    if (wait) {
        std::binary_semaphore done{0};
        {
            ...
            command_queue.emplace(
                [command = std::move(command), &done]() mutable {
                    command();
                    done.release();
                });
            ++num_commands;
...
void GpuCommandProcessor::SubmitDone() noexcept {
    std::scoped_lock lock{submit_mutex};
...
void GpuCommandProcessor::WaitGpuIdle() noexcept {
    std::unique_lock lock{submit_mutex};
    submit_cv.wait(lock, [this] { return num_submits == 0; });
}

bool GpuCommandProcessor::IsGpuIdle() const noexcept {
    return num_submits == 0;
}

void GpuCommandProcessor::ReserveCopyBufferSpace() {
    GpuQueue& gfx_queue = mapped_queues[GfxQueueId];
    std::scoped_lock lock{gfx_queue.access_mutex};
...
void GpuCommandProcessor::ProcessCommands() {
...
void GpuCommandProcessor::Process(std::stop_token stoken) {
    ...
    while (!stoken.stop_requested()) {
        ...
        while (num_submits || num_commands) {
            ...
            GpuTask::Handle task{};
            {
                std::scoped_lock lock{queue.access_mutex};
                if (queue.submissions.empty()) {
                    continue;
                }
                const auto submission = queue.submissions.front();
                task = submission.task;
                current_submission = submission.sequence;
            }
            task.resume();
...
GpuCommandProcessor::Task GpuCommandProcessor::ProcessCeUpdate(std::span<const u32> ccb) {
...
GpuCommandProcessor::Task GpuCommandProcessor::ProcessGraphics(std::span<const u32> dcb,
    ...
    while (!dcb.empty()) {
        ...
        case 3:
            const u32 count = header->type3.NumWords();
            const PM4ItOpcode opcode = header->type3.opcode;
            current_packet_offset_dw = static_cast<u32>(
                (reinterpret_cast<uintptr_t>(header) - base_addr) / sizeof(u32));
            current_packet_opcode = static_cast<u32>(opcode);
            switch (opcode) {
            case PM4ItOpcode::Nop: {
                const auto* nop = reinterpret_cast<const PM4CmdNop*>(header);
            ...
            case PM4ItOpcode::SetConfigReg: {
                ...
                register_file.WriteConfig(set_data->reg_offset, {payload, count - 1});
                break;
            }
            case PM4ItOpcode::SetContextReg: {
                ...
                register_file.WriteContext(set_data->reg_offset, {payload, count - 1});
            ...
            case PM4ItOpcode::SetShReg: {
                ...
                if (set_data->reg_offset >= 0x200 &&
                    ...
                    auto* addr = reinterpret_cast<u32*>(&mapped_queues[GfxQueueId].compute_state) +
                                 (set_data->reg_offset - 0x200);
                    std::memcpy(addr, header + 2, set_size);
                } else {
                    const auto* payload = reinterpret_cast<const u32*>(header + 2);
                    register_file.WriteShader(set_data->reg_offset, {payload, set_size / sizeof(u32)});
                }
                break;
            }
            case PM4ItOpcode::SetUconfigReg: {
                const auto* set_data = reinterpret_cast<const PM4CmdSetData*>(header);
                const auto* payload = reinterpret_cast<const u32*>(header + 2);
                register_file.WriteUconfig(set_data->reg_offset, {payload, count - 1});
                break;
            }
            ...
            case PM4ItOpcode::DrawIndex2: {
                ...
                if (command_sink) {
                    const VideoCore::DrawCommand command{
                        .provenance = CaptureProvenance(reinterpret_cast<VAddr>(header)),
            ...
            case PM4ItOpcode::DrawIndexOffset2: {
                ...
                if (command_sink) {
                    const VideoCore::DrawCommand command{
                        ...
                        .state = CaptureGraphicsState(),
                        .is_indexed = true,
                        .index_offset = draw_index_off->index_offset,
                    };
                    const auto cmd_address = reinterpret_cast<const void*>(header);
                    if (host_markers_enabled) {
                        command_sink->ScopeMarkerBegin(
            ...
            case PM4ItOpcode::DrawIndirect: {
                ...
                if (command_sink) {
                    const VideoCore::DrawIndirectCommand command{
                        .provenance = CaptureProvenance(reinterpret_cast<VAddr>(header)),
                        .state = CaptureGraphicsState(),
                        .is_indexed = false,
                        .argument_address = indirect_args_addr,
                        .offset = offset,
                        .stride = stride,
                        .max_count = 1,
                    };
            ...
            case PM4ItOpcode::DrawIndirectMulti: {
                ...
                if (command_sink) {
                    const VideoCore::DrawIndirectCommand command{
                        ...
                        .argument_address = indirect_args_addr,
                        .offset = offset,
                        .stride = draw_indirect->stride,
                        .max_count = draw_indirect->count,
                    };
                    const auto cmd_address = reinterpret_cast<const void*>(header);
                    if (host_markers_enabled) {
                        command_sink->ScopeMarkerBegin(
            ...
            case PM4ItOpcode::DrawIndexIndirectMulti: {
                ...
                if (command_sink) {
                    const VideoCore::DrawIndirectCommand command{
                        ...
                        .argument_address = indirect_args_addr,
                        .offset = offset,
                        .stride = draw_index_indirect->stride,
                        .max_count = draw_index_indirect->count,
                    };
                    const auto cmd_address = reinterpret_cast<const void*>(header);
                    if (host_markers_enabled) {
                        command_sink->ScopeMarkerBegin(
            ...
            case PM4ItOpcode::DrawIndexIndirectCountMulti: {
                ...
                if (command_sink) {
                    const VideoCore::DrawIndirectCommand command{
                        ...
                        .stride = draw_index_indirect->stride,
                        .max_count = draw_index_indirect->count,
                        .count_address = draw_index_indirect->count_indirect_enable.Value()
            ...
            case PM4ItOpcode::DispatchDirect: {
                ...
                if (command_sink && (cs_program.dispatch_initiator & 1)) {
                    const VideoCore::DispatchDirectCommand command{
                        .provenance = CaptureProvenance(reinterpret_cast<VAddr>(header)),
                        .state = CaptureComputeState(),
                    };
                    const auto cmd_address = reinterpret_cast<const void*>(header);
                    if (host_markers_enabled) {
                        command_sink->ScopeMarkerBegin(
            ...
            case PM4ItOpcode::DispatchIndirect: {
                ...
                if (command_sink && (cs_program.dispatch_initiator & 1)) {
                    const VideoCore::DispatchIndirectCommand command{
                        .provenance = CaptureProvenance(reinterpret_cast<VAddr>(header)),
                        .state = CaptureComputeState(),
                        .argument_address = indirect_args_addr,
                        .offset = offset,
            ...
            case PM4ItOpcode::DmaData: {
                ...
                const auto provenance =
                    CaptureProvenance(reinterpret_cast<VAddr>(header));
                if (dma_data->src_sel == DmaDataSrc::Data &&
                    dma_data->dst_sel == DmaDataDst::Gds) {
                    command_sink->Execute(VideoCore::FillBufferCommand{
                        .provenance = provenance,
                        .address = dma_data->dst_addr_lo,
                        .size = dma_data->NumBytes(),
                        .value = dma_data->data,
                        .is_gds = true,
                    });
                           ...
                           dma_data->dst_sel == DmaDataDst::Gds) {
                    command_sink->Execute(VideoCore::CopyBufferCommand{
                        .provenance = provenance,
                        .destination = dma_data->dst_addr_lo,
                        .source = dma_data->SrcAddress<VAddr>(),
                        .size = dma_data->NumBytes(),
                        .destination_is_gds = true,
                        .source_is_gds = false,
                    });
                    ...
                    command_sink->Execute(VideoCore::FillBufferCommand{
                        .provenance = provenance,
                        .address = dma_data->DstAddress<VAddr>(),
                        .size = dma_data->NumBytes(),
                        .value = dma_data->data,
                        .is_gds = false,
                    });
                    ...
                    command_sink->Execute(VideoCore::CopyBufferCommand{
                        .provenance = provenance,
                        .destination = dma_data->DstAddress<VAddr>(),
                        .source = dma_data->src_addr_lo,
                        .size = dma_data->NumBytes(),
                        .destination_is_gds = false,
                        .source_is_gds = true,
                    });
                } else if ((dma_data->src_sel == DmaDataSrc::Memory ||
                            dma_data->src_sel == DmaDataSrc::MemoryUsingL2) &&
            ...
            case PM4ItOpcode::WaitRegMem: {
                ...
                if (vo_port->IsVoLabel(wait_addr) &&
                    num_submits == mapped_queues[GfxQueueId].submissions.size()) {
...
GpuCommandProcessor::Task GpuCommandProcessor::ProcessCompute(std::span<const u32> acb,
                                                              u32 vqid) {
    ...
    while (!acb.empty()) {
        ...
        const PM4ItOpcode opcode = header->type3.opcode;
        const VAddr packet_address = reinterpret_cast<VAddr>(header);
        current_packet_offset_dw =
            packet_address >= base_addr && packet_address < base_addr + acb_size
                ? static_cast<u32>((packet_address - base_addr) / sizeof(u32))
                : 0;
        current_packet_opcode = static_cast<u32>(opcode);
        ...
        case PM4ItOpcode::DmaData: {
            ...
            const auto provenance = CaptureProvenance(reinterpret_cast<VAddr>(header));
            if (dma_data->src_sel == DmaDataSrc::Data &&
                dma_data->dst_sel == DmaDataDst::Gds) {
                command_sink->Execute(VideoCore::FillBufferCommand{
                    .provenance = provenance,
        ...
        case PM4ItOpcode::SetShReg: {
            ...
            if (set_data->reg_offset >= 0x200 &&
                set_data->reg_offset <= (0x200 + sizeof(ComputeProgram) / 4)) {
                ASSERT(set_size <= sizeof(ComputeProgram));
                auto* addr = reinterpret_cast<u32*>(&mapped_queues[vqid + 1].compute_state) +
                             (set_data->reg_offset - 0x200);
                std::memcpy(addr, header + 2, set_size);
            } else {
                const auto* payload = reinterpret_cast<const u32*>(header + 2);
                register_file.WriteShader(set_data->reg_offset,
                                          {payload, set_size / sizeof(u32)});
            }
            break;
        }
            ...
            if (auto it = std::ranges::find(indirect_patches, header, &IndirectPatch::header);
                ...
                if (command_sink) {
                    const VideoCore::DispatchIndirectCommand command{
                        .provenance = CaptureProvenance(reinterpret_cast<VAddr>(header)),
                        .state = CaptureComputeState(),
                        .argument_address = it->indirect_addr,
                        .size = sizeof(PM4CmdDispatchIndirect::GroupDimensions),
                    };
                    command_sink->Execute(command);
                }
                break;
        ...
        case PM4ItOpcode::ReleaseMem: {
            ...
            const auto provenance = command_sink
                                        ? CaptureProvenance(reinterpret_cast<VAddr>(header))
                                        : VideoCore::CommandProvenance{};
            release_mem->SignalFence(
                [pipe_id = queue.pipe_id] {
                    Platform::IrqC::Instance()->Signal(static_cast<Platform::InterruptId>(pipe_id));
                },
                [this, provenance](VAddr dst, u16 gds_index, u16 num_dwords) {
                    if (!command_sink) {
                        return;
                    }
                    command_sink->Execute(VideoCore::CopyBufferCommand{
                        ...
                        .destination = dst,
                        .source = gds_index,
                        .size = u32(num_dwords) * u32(sizeof(u32)),
                        .destination_is_gds = false,
                        .source_is_gds = true,
                    });
                });
...
GpuCommandProcessor::CmdBuffer GpuCommandProcessor::CopyCmdBuffers(
...
void GpuCommandProcessor::SubmitGfx(std::span<const u32> dcb, std::span<const u32> ccb) {
    auto& queue = mapped_queues[GfxQueueId];
    ...
    auto task = ProcessGraphics(dcb, ccb);
    {
        std::scoped_lock lock{queue.access_mutex};
        const u64 sequence =
            submission_sequence.fetch_add(1, std::memory_order_relaxed) + 1;
        queue.submissions.push({task.handle, sequence});
    }
...
void GpuCommandProcessor::SubmitAsc(u32 gnm_vqid, std::span<const u32> acb) {
