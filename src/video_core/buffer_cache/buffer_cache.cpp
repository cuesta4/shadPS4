...
template <typename T>
using ResolvedVertexInputs =
    boost::container::small_vector<T, Vulkan::VertexInputRequest::InlineBindings>;

BufferCache::BufferCache(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_,
                         AmdGpu::GpuThreadDispatcher* gpu_thread_dispatcher_,
                         TransferBufferPool& transfer_buffers_,
                         ResourceAliasCoordinator& alias_coordinator_, PageManager& tracker,
                         BufferAccessStateStore& access_states_)
    : instance{instance_}, scheduler{scheduler_}, gpu_thread_dispatcher{gpu_thread_dispatcher_},
      memory{Core::Memory::Instance()}, transfer_buffers{transfer_buffers_},
      alias_coordinator{alias_coordinator_}, access_states{access_states_},
      fault_manager{instance, scheduler, *this, CACHING_PAGEBITS, CACHING_NUMPAGES},
      staging_buffer{transfer_buffers.Get(MemoryUsage::Upload)},
      stream_buffer{transfer_buffers.Get(MemoryUsage::Stream)},
      download_buffer{transfer_buffers.Get(MemoryUsage::Download)},
      device_buffer{transfer_buffers.Get(MemoryUsage::DeviceLocal)},
...
void BufferCache::ReadMemory(VAddr device_addr, u64 size, bool is_write) {
    gpu_thread_dispatcher->ExecuteOnGpuThread([this, device_addr, size, is_write] {
...
void BufferCache::DownloadBufferMemory(Buffer& buffer, VAddr device_addr, u64 size, bool is_write) {
    ...
    Vulkan::ResourceCommandRecorder::CopyBuffer(
        scheduler, buffer.buffer, download_buffer.Handle(), copies);
...
Vulkan::VertexInputPlan BufferCache::PrepareVertexBuffers(
    const Vulkan::VertexInputRequest& request, Vulkan::BufferAccessPlan& accesses) {
    Vulkan::VertexInputPlan plan{};
    plan.use_dynamic_vertex_input = request.use_dynamic_vertex_input;
    plan.attributes.assign(request.attributes.begin(), request.attributes.end());
    plan.bindings.assign(request.bindings.begin(), request.bindings.end());
    ...
    ResolvedVertexInputs<BufferRange> ranges{};
    for (const auto& buffer : request.guest_buffers) {
        if (buffer.GetSize() > 0) {
    ...
    ResolvedVertexInputs<BufferRange> ranges_merged{};
    if (!ranges.empty()) {
    ...
    for (auto& range : ranges_merged) {
        ...
        const auto resolved =
            ResolveBuffer(range.base_address, static_cast<u32>(size), false, false);
        auto* buffer = resolved.buffer;
    ...
    // Bind vertex buffers
    plan.buffers.reserve(request.guest_buffers.size());
    plan.offsets.reserve(request.guest_buffers.size());
    plan.sizes.reserve(request.guest_buffers.size());
    plan.strides.reserve(request.guest_buffers.size());
    for (const auto& buffer : request.guest_buffers) {
        if (buffer.GetSize() > 0) {
            ...
            plan.buffers.emplace_back(host_buffer_info->vk_buffer);
            plan.offsets.push_back(host_buffer_info->offset + buffer.base_address -
                                   host_buffer_info->base_address);
        } else {
            plan.buffers.emplace_back(VK_NULL_HANDLE);
            plan.offsets.push_back(0);
        }
        plan.sizes.push_back(buffer.GetSize());
        plan.strides.push_back(buffer.GetStride());
    }
    return plan;
}

Vulkan::IndexInputPlan BufferCache::PrepareIndexBuffer(
    const Vulkan::IndexInputRequest& request, Vulkan::BufferAccessPlan& accesses) {
    const auto resolved = ResolveBuffer(request.address, request.size, false, false);
    auto* vk_buffer = resolved.buffer;
    const u32 offset = resolved.offset;
    if (IsRegionGpuModified(request.address, request.size)) {
        accesses.push_back({
            .resource = vk_buffer,
            .id = resolved.id,
            .generation = resolved.generation,
            .resource_uid = vk_buffer->Uid(),
...
void BufferCache::FillBuffer(VAddr address, u32 num_bytes, u32 value, bool is_gds) {
    ...
    if (!is_gds) {
        alias_coordinator.ClearMetadata(address);
...
void BufferCache::CopyBuffer(VAddr dst, VAddr src, u32 num_bytes, bool dst_gds, bool src_gds) {
    if (!dst_gds && !IsRegionGpuModified(dst, num_bytes)) {
        if (!src_gds && !IsRegionGpuModified(src, num_bytes) &&
            !alias_coordinator.HasImage(src, num_bytes)) {
    ...
    alias_coordinator.InvalidateImagesFromGpu(dst, num_bytes);
    ...
    Vulkan::ResourceCommandRecorder::CopyBuffer(
        scheduler, src_buffer.Handle(), dst_buffer.Handle(), region);
...
BufferCache::ResolvedBuffer BufferCache::ResolveBuffer(VAddr device_addr, u32 size,
                                                       bool is_written,
    ...
    BufferId id = LookupBuffer(device_addr, size);
    if (!id) {
        const auto* page = page_table.find(device_addr >> CACHING_PAGEBITS);
        if (page == nullptr || !page->buffer_id ||
            !slot_buffers[page->buffer_id].IsInBounds(device_addr, size)) {
            id = CreateBuffer(device_addr, size);
        } else {
            id = page->buffer_id;
        }
    }
    ...
    return {
        .buffer = &buffer,
        .id = id,
        .generation = alias_coordinator.Buffers().Generation(id),
        .offset = static_cast<u32>(buffer.Offset(device_addr)),
    };
}
...
BufferId BufferCache::LookupBuffer(VAddr device_addr, u32 size) const {
    ...
    const BufferId buffer_id = page->buffer_id;
    const Buffer& buffer = slot_buffers[buffer_id];
    return buffer.IsInBounds(device_addr, size) ? buffer_id : BufferId{};
}

BufferId BufferCache::FindBuffer(VAddr device_addr, u32 size) {
    ASSERT(device_addr != 0);
    if (const BufferId existing = LookupBuffer(device_addr, size)) {
        return existing;
    }
...
void BufferCache::JoinOverlap(BufferId new_buffer_id, BufferId overlap_id,
    ...
    Vulkan::ResourceCommandRecorder::CopyBuffer(
        scheduler, overlap.Handle(), new_buffer.Handle(), copy);
...
void BufferCache::ChangeRegister(BufferId buffer_id) {
    ...
    if constexpr (insert) {
        ...
        const u32 generation = alias_coordinator.Buffers().RegisterBuffer(buffer_id);
        buffer.BindAccessState(access_states, buffer_id, generation);
    } else {
        buffer.UnbindAccessState();
        alias_coordinator.Buffers().UnregisterBuffer(buffer_id);
        total_used_memory -= Common::AlignUp(size, CACHING_PAGESIZE);
...
bool BufferCache::SynchronizeBuffer(Buffer& buffer, VAddr device_addr, u32 size, bool is_written,
    ...
    if (src_buffer) {
        ...
        Vulkan::ResourceCommandRecorder::CopyBuffer(
            scheduler, src_buffer, buffer.buffer, copies);
        Vulkan::ResourceCommandRecorder::PipelineBarrier2(scheduler, vk::DependencyInfo{
            .dependencyFlags = vk::DependencyFlagBits::eByRegion,
            .bufferMemoryBarrierCount = 1,
...
bool BufferCache::SynchronizeBufferFromImage(Buffer& buffer, VAddr device_addr, u32 size) {
    return alias_coordinator.SynchronizeBufferFromImage(buffer, device_addr, size);
}
...
void BufferCache::WriteDataBuffer(Buffer& buffer, VAddr address, const void* value, u32 num_bytes) {
    ...
    Vulkan::ResourceCommandRecorder::CopyBuffer(
        scheduler, src_buffer, buffer.Handle(), copy);
    Vulkan::ResourceCommandRecorder::PipelineBarrier2(scheduler, vk::DependencyInfo{
        .dependencyFlags = vk::DependencyFlagBits::eByRegion,
        .bufferMemoryBarrierCount = 1,
        .pBufferMemoryBarriers = &post_barrier,
